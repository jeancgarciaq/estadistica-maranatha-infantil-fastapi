...
class ReporteScreen(Screen):
    estado_texto = StringProperty('Seleccione la fecha del domingo y genere el PDF.')
    ...
    def generar_pdf(self):
        ...
        try:
            ...
            self.estado_texto = f'PDF generado en: {pdf}'
            self._ultimo_pdf = pdf
            StyledPopup.mostrar_popup('Éxito', 'PDF generado correctamente.', tipo='success')
        except Exception as e:
            logger.exception('Error al generar PDF')
            StyledPopup.mostrar_popup('Error', f'No se pudo generar el PDF: {e}', tipo='error')
