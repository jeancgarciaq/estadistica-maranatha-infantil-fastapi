...
class Auxiliar(Base, AuditMixin):
    ...
    area = relationship("Area", backref="auxiliares")

    id_capitan = Column(Integer, ForeignKey('capitanes.id'), nullable=True)
    capitan = relationship("Capitan", backref="auxiliares")

    def __repr__(self):
        return f"<Auxiliar(id={self.id}, nombre='{self.nombre}', cedula='{self.cedula}')>"
